...
const getAll = async () => {
  const queryString = `SELECT ORDERS.*, EMPLOYEE.NAME, CUSTOMER.FULLNAME, PRODUCT.NAME  AS 'PRODUCT_NAME', PRODUCT.PRICE FROM (((CUSTOMER INNER JOIN ORDERS ON CUSTOMER.CUSTOMER_ID = ORDERS.CUSTOMER_ID) INNER JOIN EMPLOYEE ON ORDERS.EMPLOYEE_ID = EMPLOYEE.EMPLOYEE_ID)INNER JOIN PRODUCT ON ORDERS.PRODUCT_ID = PRODUCT.PRODUCT_ID)`;
  try {
...
const deleteById = async (id) => {
  const queryString = `DELETE ${DATABASE_NAME} WHERE ORDER_ID = '${id}'`;
  try {
    const data = await pool.request().query(queryString);
    return data;
  } catch (error) {
